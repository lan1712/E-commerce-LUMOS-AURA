...
export function AddressesPage() {
  ...
  const handleSave = (data: Omit<Address, "id">) => {
    if (editingId) {
      setAddresses((prev) =>
        prev.map((a) => (a.id === editingId ? { ...data, id: editingId } : data.isDefault ? { ...a, isDefault: false } : a))
      );
      setEditingId(null);
    } else {
      const newAddr: Address = { ...data, id: `addr-${Date.now()}` };
      setAddresses((prev) =>
        data.isDefault ? [newAddr, ...prev.map((a) => ({ ...a, isDefault: false }))] : [...prev, newAddr]
      );
      setShowForm(false);
    }
  };
