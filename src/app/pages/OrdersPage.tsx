...
export function OrdersPage() {
  ...
  const [orders, setOrders] = useState<typeof mockOrders>(mockOrders);
  const [selectedOrder, setSelectedOrder] = useState<typeof mockOrders[0]>(mockOrders[0]);
  ...
  useEffect(() => {
    ...
    ordersApi.list()
      .then((data: Array<{
        orderNumber: string; createdAt: string; total: number; status: string;
        items: Array<{ productName: string; price: number; quantity: number; imageUrl?: string }>;
      }>) => {
        ...
        const mapped = data.map((o) => ({
          ...
          status: (o.status.charAt(0) + o.status.slice(1).toLowerCase()) as "Processing" | "Delivered" | "Pending",
          trackingStep: o.status === "DELIVERED" ? 4 : o.status === "PROCESSING" ? 2 : 1,
          items: (o.items ?? []).map((i) => ({
            name: i.productName,
            desc: "",
